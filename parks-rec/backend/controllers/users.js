...
router.post('/signup', (req, res) => {
    ...
    db.User.create(req.body)
        ...
        // Send error if the database fails to create a user
        .catch((err) => {
            console.error('Error creating user:', err);

            // Send the status and the JSON in one response
            res.status(401).json({ error: 'Could not create a new user, try again' });
        });
});
