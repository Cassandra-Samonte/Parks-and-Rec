    ...
    return (
      <>
        ...
        <div className="absolute top-0 left-0 w-full h-full flex items-center justify-center">
            <h1 className="text-white text-4xl font-bold">Parks & Rec</h1>
        </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mt-8">
          {news.filter(article => article.image && article.image.url).slice(0, 4).map((article) => (
            <a href={article.url} key={article.id} target="_blank" rel="noopener noreferrer" className="block bg-white overflow-hidden">
